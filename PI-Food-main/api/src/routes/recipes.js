...
const getRecipeApiInfo= async()=>{
    const apiUrlSearch=await axios.get (`https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&addRecipeInformation=true&number=50`);
...
const getNameByApi= async(name)=>{
  const nameAxiosApi= await axios.get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&query=${name}&addRecipeInformation=true&number=100`);
...
router.get('/:id',async(req,res)=>{
    const {id}=req.params;
    if (id.length<10) {
        const searchId= await axios.get(`https://api.spoonacular.com/recipes/${id}/information?apiKey=${API_KEY}`)
